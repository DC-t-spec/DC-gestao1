...
(() => {
  ...
  /* =======================
     Online config (Supabase) — faltava no teu JS
     (Guarda no localStorage ANTES do login)
  ======================= */
...
function setOnlineConfig(url, key) {
  ...
  /* =======================
     MERGE helpers (multi-dispositivo)
  ======================= */
  function toMs(v) {
    if (v == null) return 0;
  ...
  function baseIdForProduct(productId) {
    ...
    if (!p) return null;
    return p.stockBaseId && String(p.stockBaseId).trim() ? p.stockBaseId : p.id;
  }
  ...
  async function setSecurityQA(userId, question, answerPlain) {
    ...
    db.users = db.users.map((u) =>
      u.id !== userId ? u : { ...u, securityQuestion: q, securityAnswerHash: hash, updatedAt: Date.now() }
    );
  ...
  function adminResetPin(userId, tempPin) {
    ...
    db.users = db.users.map((u) =>
      u.id !== userId ? u : { ...u, pin: String(tempPin).trim(), mustChangePin: true, ativo: true, updatedAt: Date.now() }
    );
  ...
  function changeMyPin(oldPin, newPin) {
    ...
    db.users = db.users.map((x) =>
      x.id !== u.id ? x : { ...x, pin: String(newPin).trim(), mustChangePin: false, updatedAt: Date.now() }
    );
  ...
  async function recoverPinByQuestion(userId, answerPlain, newPin) {
    ...
    db.users = db.users.map((x) =>
      x.id !== userId ? x : { ...x, pin: String(newPin).trim(), mustChangePin: false, updatedAt: Date.now() }
    );
  ...
  /* =======================
     RELATÓRIOS VISUAIS (12) — mantive igual ao teu
  ======================= */
  ...
  function renderWorkspaceBadge() {
    ...
    el.textContent = nome ? `🏪 ${nome} • ${ws}` : `🏪 Workspace: ${ws}`;
  }
...
document.addEventListener("click", (e) => {
  ...
  alert("✅ O botão Carregar da Nuvem foi clicado!");
    ...
    if (btnCmp) btnCmp.addEventListener("click", () => modalCompanySetup());
    ...
    const cfgNow = getOnlineConfig();
    if (sbUrl) sbUrl.value = cfgNow.url || "";
    if (sbKey) sbKey.value = cfgNow.key || "";
    ...
    document.addEventListener("click", (e) => {
      ...
      if (togProd) {
        ...
        db.products = db.products.map((p) => (p.id === id ? { ...p, ativo: !p.ativo, updatedAt: Date.now() } : p));
        touch();
        renderAll();
        return;
      }
      ...
      if (uToggle) {
        ...
        db.users = db.users.map((u) => (u.id === id ? { ...u, ativo: u.ativo === false ? true : false, updatedAt: Date.now() } : u));
        touch();
        renderAll();
        return;
      }
    ...
    alert("Erro ao carregar da nuvem: " + (err?.message || err));
      ...
      if (buyForm) {
        ...
        db.products = db.products.map((p) =>
          p.id === productId ? { ...p, precoAquisicaoRef: costUnit, updatedAt: Date.now() } : p
        );
